...
В рамках данного спринта создается пользовательский интерфейс для сервиса, созданного на спринте 2. Интерфейс размещается на веб странице и имеет две вкладки:

1. Предсказание стоимости квадратного метра для продажи в рублях.
2. Проверка соответствия требованиям сетевых арендаторов.

### Предсказание стоимости квадратного метра для продажи

Первая вкладка следующие поля входных данных:
...
### Проверка соответствия требованиям сетевых арендаторов.

Вторая вкладка имеет две подвкладки: 1) для проверки зданий и помещений 2) для проверки земельных участков.

Первая подвкладка имеет следующие поля входных данных:

| Название поля                                  | Тип данных         | Пример                            | Значение по-умолчанию              | Обязательное поле  |
|------------------------------------------------|--------------------|-----------------------------------|------------------------------------|--------------------|
| Общая площадь (м²)                             | С точкой           | 50.0                              | 30.0                               | Да                |
| Этаж                                           | Целочисленное      | 5                                 | 2                                  | Да                |
| Близость жилого сектора, высокий пешеходный трафик | Категориальный            | Да                              |                               | Нет                |
| Высокий автомобильный трафик                   | Категориальный             | Да                              |                               | Нет                |
| В 100 метрах отсутствуют детские, образовательные, спортивные, медицинские объекты | Категориальный            | Да                              |                               | Нет                |
| Наличие всех коммуникаций                      | Категориальный             | Да                              |                               | Нет                 |
| Наличие сан. узла                              | Категориальный             | Да                              |                               | Нет                |
| От 20 тыс. посетителей в день (для ТЦ)          | Категориальный      | Да                             | Нет                                  | Нет                |
| Возможность разгрузки грузового транспорта      | Категориальный             | Да                              |                               | Нет                |
| Минимальная высота потолков в м.                | С точкой           | 3.0                               | 2.5                                | Нет                 |
| Наличие парковки                               | Категориальный             | Да                              |                               | Нет                |


Вторая подвкладка имеет следующие поля входных данных:

| Название поля                | Тип данных        | Пример                    | Значение по-умолчанию          | Обязательное поле  |
|------------------------------|-------------------|---------------------------|--------------------------------|--------------------|
| Общая площадь (м²)           | С точкой          | 500.0                      | 100.0                           | Да                 |
| Близость к жилому сектору с высоким пешеходным трафиком                         | Категориальный     | Да                         |                              | Нет                 |
| Высокий автомобильный трафик                         | Категориальный     | Да                         |                               | Нет                 |
| Наличие всех коммуникаций                         | Категориальный     | Да                         |                              | Нет                 |

В качестве результата пользователь получает категорию и перечень сетей для каждой категории, которые удовлетворяют запросу.

Цена: 15000 руб, сроки: 1.5 недели.
